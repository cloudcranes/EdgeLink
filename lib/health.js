...
// building 状态下的重复探测只更新内存，不每次 writeConfig；
// 只有首次进入 building、状态切换、或终态 live/failed 才持久化。
async function setAppStatus(appId, status, extras = {}) {
  ...
  const stateBefore = liveCheckState.get(appId);
  // 始终先把最新字段写回内存（extras），便于下一次 tick 看到
  if (stateBefore) {
    if (extras.lastCheckedAt) stateBefore.lastCheckedAt = extras.lastCheckedAt;
    if (extras.lastError !== undefined) stateBefore.lastError = extras.lastError;
    if (extras.lastUrl) stateBefore.lastUrl = extras.lastUrl;
  }
  const changed = prev !== status;
  // building → building 的重复轮询：仅更新 liveCheckState 内存，跳过 writeConfig
  const skipPersist = !changed && status === 'building';
  app.status = status;
  ...
  // 服务端日志：状态变化时记录；building→building 不记（避免刷屏）
  if (changed) {
