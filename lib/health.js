...
async function probeUrl(url) {
  ...
  try {
    ...
    if (response.ok) {
      return { ok: true, status: response.status, latency: Date.now() };
    }
  ...
  try {
    ...
    return { ok: response.ok, status: response.status, latency: Date.now() };
  } catch (error2) {
    return { ok: false, error: error2.message || String(error2), latency: Date.now() };
  }
}
