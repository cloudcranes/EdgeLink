...
// 读 10s / 写 20s（与前端 lucky 默认对齐）；登录专用 10s（独立传 timeoutMs）。
const LUCKY_TIMEOUT_GET_MS = 10_000;
const LUCKY_TIMEOUT_WRITE_MS = 20_000;
const LUCKY_TIMEOUT_LOGIN_MS = 10_000;

function timeoutForMethod(method) {
  return method === 'GET' || method === 'HEAD' ? LUCKY_TIMEOUT_GET_MS : LUCKY_TIMEOUT_WRITE_MS;
}

// 把 AbortSignal.timeout 抛出的 AbortError / TimeoutError 翻译成稳定的 ETIMEDOUT 错误。
// timeoutMs 可选：登录（POST /api/login）专用 10s；其余 luckyRequest 走 method 默认值。
// 其它网络错误不动，让上游看到原始 cause（DNS/连接拒绝等）。
function translateAbortError(error, method, apiPath, timeoutMs) {
  if (error && (error.name === 'AbortError' || error.name === 'TimeoutError')) {
    const ms = Number.isInteger(timeoutMs) && timeoutMs > 0 ? timeoutMs : timeoutForMethod(method);
    const e = new Error(`Lucky ${method} ${apiPath} 超时（>${ms / 1000}s）`);
    e.code = 'ETIMEDOUT';
    e.status = 504;
    return e;
  }
...
async function luckyLogin(config) {
  ...
  try {
    response = await fetch(`${baseUrl}/api/login`, {
      ...
      signal: AbortSignal.timeout(LUCKY_TIMEOUT_LOGIN_MS),
      body: JSON.stringify({
    ...
    throw translateAbortError(error, 'POST', '/api/login', LUCKY_TIMEOUT_LOGIN_MS);
  }
...
async function luckyRequest(baseUrl, token, method, apiPath, body) {
  ...
  try {
    response = await fetch(`${baseUrl}${apiPath}`, {
      ...
      signal: AbortSignal.timeout(timeoutForMethod(method)),
      body: body ? JSON.stringify(body) : undefined,
    });
  } catch (error) {
    throw translateAbortError(error, method, apiPath);
  }
  const data = await readJsonResponse(response);
